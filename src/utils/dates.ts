...
import { TzDateDto } from "../shared/dto/tz-date.dto";
...
export function getDateAndTimeZoneString({ datetime, timezone }: TzDateDto): { date: string; time: string } {
  if (typeof datetime === "string") {
    datetime = new Date(datetime);
  }
...
export function getLocalDateString({ datetime, timezone }: TzDateDto): string {
  if (typeof datetime === "string") {
    datetime = new Date(datetime);
  }
